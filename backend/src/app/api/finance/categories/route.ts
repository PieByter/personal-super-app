...
export async function PUT(req: NextRequest) {
    ...
    try {
        ...
        const body = await req.json();
        const data = categorySchema.parse(body);
        const [updated] = await db.update(financeCategories).set({ name: data.name, type: data.type, color: data.color, icon: data.icon, parentId: data.parentId }).where(and(eq(financeCategories.id, id), eq(financeCategories.userId, user.userId))).returning();
...
export async function DELETE(req: NextRequest) {
    ...
    try {
        const id = new URL(req.url).searchParams.get("id");
        if (!id) return Response.json({ error: "ID required" }, { status: 400 });
        await db.delete(financeCategories).where(and(eq(financeCategories.id, id), eq(financeCategories.userId, user.userId)));
